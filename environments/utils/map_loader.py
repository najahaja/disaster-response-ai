...
# ✅ Define SyntheticMap at module level (so it can be pickled)
class SyntheticMap:
    def __init__(self, grid: np.ndarray, location: str):
        self.grid = grid
...
class MapLoader:
    ...
    def _load_cached_map(self, location_name: str) -> Optional[Any]:
        ...
        if os.path.exists(cache_file) and os.path.getsize(cache_file) > 0:
            ...
            logger.warning(f"No valid cache file found for {location_name}")

        return None

    def _generate_synthetic_map(self, location_name: str) -> SyntheticMap:
        """Generate a synthetic city map"""
        logger.info(f"Generating synthetic map for: {location_name}")

        grid_size = 30
        grid = np.full((grid_size, grid_size), 1)  # Default: buildings
        ...
        # Create synthetic map and cache it
        synthetic_map = SyntheticMap(grid, location_name)
